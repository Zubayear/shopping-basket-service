...
func (eq *EventQuery) QueryOwner() *BasketLineQuery {
	query := &BasketLineQuery{config: eq.config}
...
func (eq *EventQuery) WithOwner(opts ...func(*BasketLineQuery)) *EventQuery {
	query := &BasketLineQuery{config: eq.config}
...
func (eq *EventQuery) sqlAll(ctx context.Context) ([]*Event, error) {
	...
	if query := eq.withOwner; query != nil {
		...
		for i := range nodes {
			if nodes[i].basket_line_event == nil {
				continue
			}
			fk := *nodes[i].basket_line_event
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(basketline.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "basket_line_event" returned %v`, n.ID)
			}
			for i := range nodes {
