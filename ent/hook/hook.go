...
// The BasketLineFunc type is an adapter to allow the use of ordinary
// function as BasketLine mutator.
type BasketLineFunc func(context.Context, *ent.BasketLineMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f BasketLineFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.BasketLineMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.BasketLineMutation", m)
	}
	return f(ctx, mv)
}
